...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["find-IncG1"])) {
    ...
    try {
        ...
        if(empty($SearchSuffix) && empty($SearchMiddname)){
            $query = "SELECT *
                  FROM transferee
                  WHERE TR_First_Name LIKE :Search1Stname AND TR_Last_Name LIKE :SearchLastname AND Grade_level = :Glevel;";
            ...
            $query = "SELECT *
                  FROM transferee
                  WHERE TR_First_Name LIKE :Search1Stname AND TR_Middle_Name LIKE :SearchMiddname AND TR_Last_Name LIKE :SearchLastname AND TR_Suffix LIKE :SearchSuffix AND Grade_level = :Glevel;";
            ...
            try{
                $query = "SELECT *
                  FROM transferee
                  WHERE TR_First_Name LIKE :Search1Stname AND TR_Last_Name LIKE :SearchLastname AND TR_Middle_Name LIKE :SearchMiddname AND Grade_level = :Glevel;";
            ...
            $query = "SELECT *
                  ...
                  WHERE TR_First_Name LIKE :Search1Stname AND TR_Middle_Name LIKE :SearchMiddname AND TR_Last_Name LIKE :SearchLastname AND Grade_level = :Glevel;";
                ...
                <div class="boxes">
                    ...
                    <?php
                        ...
                        if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST["find-IncG1"])) {
                            ...
                            if (!preg_match("/^[a-zA-Z\s]+$/", $NAME1ST)) {
                                $NAME1ST_error = 'Only Text is Allowed';
                            }  
                            
                            if (!empty($SearchMiddname)) {
                                if (!preg_match("/^[a-zA-Z\s]+$/", $SearchMiddname)) {
                                    $NAMEMid_error = 'Only Text is Allowed';
                                } 
                            }
                            
                            if (!preg_match("/^[a-zA-Z\s]+$/", $SearchLastname)) {
